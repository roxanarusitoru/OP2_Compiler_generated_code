...
def addLinesToFileAndRemoveFromList(file_lines, gen_lines):
  ...
  while final_line-initial_line > 0:
    if not isRogueLine(gen_lines[initial_line]):
      file_lines.append(gen_lines[initial_line]);
    gen_lines.remove(gen_lines[initial_line]);
    final_line -= 1;
  return [file_lines, gen_lines];

def isRogueLine(line):
  return (string.find(line, 'typedef ') != -1 and string.find(line, 'int') != -1 and string.find(line, 'float') != -1);
openCurlyBraces = 0;
closedCurlyBraces = 0;
...
for line in gen_lines:
  if isIfDefStatement(line):
    ...
    while not isEndIfDefStatement(gen_lines[line_index]):
      line_index += 1;
    while line_index - initial_line > 0:
...
hosts_file = open('rose_opencl_hosts.cpp','w');
kernels_file = open('rose_opencl_code_opencl.cl','w');
