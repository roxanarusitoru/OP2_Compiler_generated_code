...
void op_par_loop_bres_calc(const char *userSubroutine,op_set set,op_arg opDat1,op_arg opDat2,op_arg opDat3,op_arg opDat4,op_arg opDat5,op_arg opDat6, op_tuner *tuner)
{
...
/*    for(int m = 0; m < 6; ++m) {
  ...
  for (i3 = 0; i3 < planRet -> ncolors; ++i3) {
    ...
    kernelPointer = getKernel("op_cuda_bres_calc");
    ...
    errorCode = errorCode | clSetKernelArg(kernelPointer,23,sizeof(cl_mem ),&g_const_d);
